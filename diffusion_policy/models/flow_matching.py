...
from .diffusion_policy import SimpleTextEncoder
...
class FlowMatchingPolicy(nn.Module):
    ...
    def forward(self, nimage, nagent_pos, naction, ntext = None, p_uncond = 0.1):
        ...
        obs_cond = self.get_cond(nimage, nagent_pos, ntext)
    ...
    def get_cond(self, nimage, nagent_pos, ntext):
        """
        ...
        if self.text:
            text_emb = self.text_encoder(ntext)
            obs_cond = torch.cat([obs_cond, text_emb], dim=-1)
            
        return obs_cond
    
    @torch.no_grad()
    def predict_action(self, nimage, nagent_pos, ntext=None, num_inference_steps=50):
        """
        ...
        for i in range(num_inference_steps):
            ...
            v_t = self.flow_predictor(x, timestep=t_val, global_cond=obs_cond)
