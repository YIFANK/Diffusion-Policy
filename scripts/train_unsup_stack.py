"""Auto-decoder on the paired StackThree demos: does an unsupervised
per-episode latent organize by OBJECT factors (src color, tgt color)?

Usage:
    python train_unsup_stack.py --data ../robosuite_stack_data/train.npz
"""
import sys
import os
...
from train_stack import StackDS, OBS_HORIZON, PRED_HORIZON

Z_DIM = 64


class UnsupStackDS(StackDS):
...
if __name__ == '__main__':
    ...
    with open(args.model_path.replace('.pth', '_ep_labels.json'), 'w') as f:
        json.dump([f"{m['src']}_{m['tgt']}" for m in meta], f)
    print(f'{n_eps} episodes, {len(ds)} samples', flush=True)
    ...
    print(f'STACKUNSUP_TRAIN_DONE {args.model_path}', flush=True)
