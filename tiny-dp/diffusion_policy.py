#tiny diffusion policy without CNN
import torch
...
class DiffusionPolicy(nn.Module):
    def __init__(
        ...
        self.eps_net = MLP(in_dim=act_dim + obs_dim + 128, out_dim=act_dim)
    ...
    # --------------------------- sampling API ------------------------------- #
    @torch.no_grad()
    def sample(self, obs: torch.Tensor, n: int = 1):
        """obs_img: (3,H,W) or (B,3,H,W)"""
        cond = obs
        x_t = torch.randn(n, self.act_dim, device=obs.device)
